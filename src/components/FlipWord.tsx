 ...
 * - `width` — the role boards reserve their longest word so they never reflow;
 *   the article sizes to whatever is showing, so "a" gives its spare cell back
 *   and the sentence closes up behind it rather than sitting on a blank cell.
 */
...
export function FlipWord({
  ...
  // `fixed` (the default) reserves the longest word's width, so the board holds
  // its grid whatever word is showing. `content` sizes to the word showing, so a
  // shorter one gives its spare cell back and the line closes up behind it.
  const slotCount =
    width === 'content'
      ? target.length
